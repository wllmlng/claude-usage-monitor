...
- **Monthly Calendar** — Daily cost heatmap with projected monthly spend; browse past months with `◀ ▶`
...
- **Header** — Current time, date, and timezone; turns yellow when viewing a past month

Cost estimates use per-model API pricing (all 13 Claude models) and are tracked per message — not a flat session rate.
...
Press `◀ ▶` to browse past months. Press `Ctrl+C` to exit.
