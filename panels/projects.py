...
def build_projects_panel(today_sessions, all_sessions):
    ...
    def sort_key(name):
        ...
        t_in, t_out, t_cr, t_cc = aggregate_tokens_for_dates(sessions, today, today)
        return -(t_in + t_out + t_cr + t_cc)

    for name in sorted(today_by_project.keys(), key=sort_key):
        today_s = today_by_project[name]
        all_s = all_by_project.get(name, [])

        t_in, t_out, t_cr, t_cc = aggregate_tokens_for_dates(today_s, today, today)
        today_tok = t_in + t_out + t_cr + t_cc
        all_tok = sum(
